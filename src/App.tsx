
import React, { useState, useEffect } from 'react';
import { HashRouter as Router, Routes, Route, Navigate } from 'react-router-dom';
import { createClient, Session } from '@supabase/supabase-js';
import { useTranslation } from 'react-i18next';
import { Layout } from './components/Layout.tsx';
import { BuyerGallery } from './components/BuyerGallery.tsx';
import { SellerDashboard } from './components/SellerDashboard.tsx';
import { LoginPage } from './components/LoginPage.tsx';
import { Product, Order, SellerSettings, SpecialRequest } from './types.ts';

import { withRetry } from './src/utils/supabaseRetry.ts';

// إعدادات الاتصال بـ Supabase
export const SUPABASE_URL = 'https://bggvmvrewvmwmmlvfgoj.supabase.co';
export const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJnZ3ZtdnJld3Ztd21tbHZmZ29qIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIwNDkyNzQsImV4cCI6MjA4NzYyNTI3NH0._Q0rN-2YhXw79YZh4kEn1ocnFYSnCEsgnpQd3K8Z_-E';

export const supabase = createClient(SUPABASE_URL, SUPABASE_ANON_KEY);

const App: React.FC = () => {
  const { i18n, t } = useTranslation();
  const [session, setSession] = useState<Session | null>(null);

  useEffect(() => {
    // تحديث اتجاه الصفحة بناءً على اللغة
    document.documentElement.dir = i18n.language === 'ar' ? 'rtl' : 'ltr';
    document.documentElement.lang = i18n.language;
  }, [i18n.language]);
  const [products, setProducts] = useState<Product[]>([]);
  const [orders, setOrders] = useState<Order[]>([]);
  const [specialRequests, setSpecialRequests] = useState<SpecialRequest[]>([]);
  const [settings, setSettings] = useState<SellerSettings>({
    bank_name: 'المصرف الصحاري',
    account_holder: 'ابوعجيله محمد ابوعجيله',
    account_number: '2005159787',
    iban: 'LY65006035000002005159787',
    phone_number: '0924253894'
  });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // التحقق من الجلسة الحالية عند تحميل التطبيق
    const initAuth = async (retryCount = 0) => {
      try {
        const result = await withRetry(async () => await supabase.auth.getSession());
        const { data, error: authError } = result;
        const session = data?.session;
        
        if (authError) {
          // تجاهل أخطاء القفل (Lock errors) لأنها غالباً ما تكون عابرة ويتم التعامل معها بواسطة onAuthStateChange
          if (authError.message?.includes('Lock broken') || authError.message?.includes('stole it')) {
            console.warn("Supabase Auth Lock warning (benign):", authError.message);
            // لا نفعل شيئاً، سنعتمد على onAuthStateChange
            return;
          }
          throw authError;
        }
        setSession(session || null);
        fetchInitialData();
      } catch (err: any) {
        console.error(`Auth initialization error (attempt ${retryCount + 1}):`, err);
        
        const isNetworkError = err.message === 'Failed to fetch' || !window.navigator.onLine;
        const isDbRestartError = err.message?.includes('terminating connection') || err.message?.includes('57P01');
        
        if ((isNetworkError || isDbRestartError) && retryCount < 2) {
          setTimeout(() => initAuth(retryCount + 1), 2000);
          return;
        }

        if (isNetworkError || isDbRestartError) {
          setError(isNetworkError 
            ? t('auth_connection_failed')
            : t('db_connection_cut'));
          setLoading(false);
        } else if (err.message?.includes('Lock broken') || err.message?.includes('stole it')) {
          // تجاهل الخطأ في الـ catch أيضاً إذا كان متعلقاً بالقفل
          console.warn("Ignored lock error in catch block");
        } else {
          fetchInitialData(); // محاولة جلب البيانات العامة على الأقل
        }
      }
    };

    initAuth();

    // الاستماع لتغييرات الحالة (دخول/خروج)
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      // إذا تغيرت الجلسة (مثلاً سجل المستخدم دخوله)، نقوم بتحديث البيانات
      if (_event === 'SIGNED_IN') {
        fetchInitialData(true);
      }
    });

    return () => subscription.unsubscribe();
  }, []);

  // تحديث الإعدادات لمرة واحدة في قاعدة البيانات لضمان وجود القيم المطلوبة (Idempotent)
  useEffect(() => {
    const ensureSettings = async () => {
      try {
        const { data: existing, error: getError } = await withRetry(async () => 
          await supabase.from('settings').select('*').eq('id', 1).single()
        );
        
        if (getError && getError.code !== 'PGRST116') throw getError;
        
        // إذا لم تكن الإعدادات موجودة أو كانت مختلفة، نقوم بتحديثها
        if (!existing || 
            existing.bank_name !== 'المصرف الصحاري' || 
            existing.account_holder !== 'ابوعجيله محمد ابوعجيله' || 
            existing.account_number !== '2005159787' || 
            existing.phone_number !== '0924253894' ||
            existing.iban !== 'LY65006035000002005159787') {
          
          await withRetry(async () => await supabase.from('settings').upsert([{ 
            id: 1, 
            bank_name: 'المصرف الصحاري', 
            account_holder: 'ابوعجيله محمد ابوعجيله', 
            account_number: '2005159787', 
            phone_number: '0924253894',
            iban: 'LY65006035000002005159787'
          }]));
        }
      } catch (err) {
        console.error("Error ensuring settings:", err);
      }
    };
    
    ensureSettings();
  }, []);

  const fetchInitialData = async (silent = false, retryCount = 0) => {
    try {
      if (!silent) setLoading(true);
      setError(null);
      
      // جلب المنتجات مع استخدام created_at
      const { data: prodData, error: prodError } = await withRetry(async () => 
        await supabase.from('products').select('*').order('created_at', { ascending: false })
      );
      if (prodError) throw prodError;
      if (prodData) setProducts(prodData);

      const { data: orderData, error: orderError } = await withRetry(async () => 
        await supabase.from('orders').select('*').order('timestamp', { ascending: false })
      );
      if (orderError) throw orderError;
      if (orderData) {
        setOrders(orderData);
        
        // ميزة الإصلاح التلقائي للطلبات المعتمدة التي تفتقد لرابط التحميل (Self-Healing)
        const missingLinks = orderData.filter(o => o.status === 'approved' && (!o.sale_file_url || o.sale_file_url.trim() === ''));
        if (missingLinks.length > 0) {
          const repairOrders = async () => {
            for (const order of missingLinks) {
              const product = prodData?.find(p => p.id === order.product_id);
              if (product && product.sale_file_url && product.sale_file_url.trim() !== '') {
                console.log(`Auto-repairing order ${order.id} with link from product ${product.id}`);
                const { error: repairError } = await withRetry(async () => 
                  await supabase.from('orders').update({ sale_file_url: product.sale_file_url }).eq('id', order.id)
                );
                
                if (!repairError) {
                  setOrders(prev => prev.map(o => o.id === order.id ? { ...o, sale_file_url: product.sale_file_url } : o));
                }
              }
            }
          };
          repairOrders();
        }
      }

      const { data: specData, error: specError } = await withRetry(async () => 
        await supabase.from('special_requests').select('*').order('timestamp', { ascending: false })
      );
      if (specError) throw specError;
      if (specData) setSpecialRequests(specData);

      const { data: settsData, error: settsError } = await withRetry(async () => 
        await supabase.from('settings').select('*').eq('id', 1).single()
      );
      if (settsError && settsError.code !== 'PGRST116') throw settsError;
      
      if (settsData) {
        setSettings(prev => ({
          ...prev,
          bank_name: settsData.bank_name || prev.bank_name,
          account_holder: settsData.account_holder || prev.account_holder,
          account_number: settsData.account_number || prev.account_number,
          iban: settsData.iban || prev.iban,
          phone_number: settsData.phone_number || prev.phone_number
        }));
      }
    } catch (err: any) {
      console.error(`Error loading data (attempt ${retryCount + 1}):`, err);
      
      // محاولة إعادة الاتصال تلقائياً في حالة فشل الشبكة أو إعادة تشغيل القاعدة
      const isNetworkError = err.message === 'Failed to fetch' || !window.navigator.onLine;
      const isDbRestartError = err.message?.includes('terminating connection') || err.message?.includes('57P01');
      
      if ((isNetworkError || isDbRestartError) && retryCount < 2) {
        setTimeout(() => fetchInitialData(silent, retryCount + 1), 2000);
        return;
      }

      if (!silent) {
        setError(isNetworkError 
          ? t('db_connection_failed')
          : (isDbRestartError ? t('db_maintenance') : `${t('error_occurred')}: ${err.message}`));
      }
    } finally {
      if (!silent && retryCount >= 2) setLoading(false);
      else if (!silent && !error) setLoading(false);
    }
  };

  const addProduct = async (product: Partial<Product>) => {
    try {
      // تنظيف الكائن من القيم غير المعرفة، مع الحفاظ على المعرف إذا تم توفيره
      const { created_at, ...dataToInsert } = product;
      const cleanProduct = Object.fromEntries(
        Object.entries(dataToInsert).filter(([_, v]) => v !== undefined)
      );

      const { data, error } = await withRetry(async () => 
        await supabase.from('products').insert([cleanProduct]).select().single()
      );

      if (error) {
        console.error("Error inserting product:", error);
        const msg = error.message === 'Failed to fetch' 
          ? t('network_error')
          : error.message;
        alert(`${t('add_work_failed')}: ${msg}\n\n${t('add_work_tip')}`);
      } else if (data) {
        setProducts(prev => [data as Product, ...prev]);
        return true;
      }
    } catch (err: any) {
      console.error("Add product exception:", err);
      alert(`${t('add_work_failed')}: ${err.message}`);
    }
    return false;
  };

  const deleteProduct = async (id: string) => {
    const { error } = await withRetry(async () => await supabase.from('products').delete().eq('id', id));
    if (!error) setProducts(prev => prev.filter(p => p.id !== id));
    else {
      const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
      alert(`${t('delete_failed')}: ${msg}`);
    }
  };

  const addOrder = async (order: Partial<Order>) => {
    try {
      const { timestamp, ...dataToInsert } = order;
      const cleanOrder = Object.fromEntries(
        Object.entries(dataToInsert).filter(([_, v]) => v !== undefined)
      );

      const { data, error } = await withRetry(async () => 
        await supabase.from('orders').insert([cleanOrder]).select().single()
      );

      if (error) {
        console.error("Error inserting order:", error);
        const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
        alert(`${t('send_order_failed')}: ${msg}\n\n${t('send_order_tip')}`);
      } else if (data) {
        setOrders(prev => [data as Order, ...prev]);
        return true;
      }
    } catch (err: any) {
      console.error("Add order exception:", err);
      alert(`${t('send_order_failed')}: ${err.message}`);
    }
    return false;
  };

  const updateOrderStatus = async (orderId: string, status: 'approved' | 'rejected') => {
    try {
      const order = orders.find(o => o.id === orderId);
      let updates: any = { status };

      // إذا تمت الموافقة وكان رابط الملف مفقوداً في الطلب، نحاول جلبه من المنتج الحالي تلقائياً
      if (status === 'approved' && order && (!order.sale_file_url || order.sale_file_url.trim() === '')) {
        const product = products.find(p => p.id === order.product_id);
        if (product && product.sale_file_url) {
          updates.sale_file_url = product.sale_file_url;
        }
      }

      const { error } = await withRetry(async () => await supabase.from('orders').update(updates).eq('id', orderId));
      if (error) {
        console.error("Error updating order status:", error);
        const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
        alert(`${t('update_order_status_failed')}: ${msg}`);
      } else {
        setOrders(prev => prev.map(o => o.id === orderId ? { ...o, ...updates } : o));
      }
    } catch (err) {
      console.error("Update order error:", err);
    }
  };

  const addSpecialRequest = async (req: Partial<SpecialRequest>) => {
    try {
      const { timestamp, ...dataToInsert } = req;
      const cleanReq = Object.fromEntries(
        Object.entries(dataToInsert).filter(([_, v]) => v !== undefined)
      );

      const { data, error } = await withRetry(async () => 
        await supabase.from('special_requests').insert([cleanReq]).select().single()
      );

      if (error) {
        console.error("Error inserting special request:", error);
        const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
        alert(`${t('send_special_request_failed')}: ${msg}\n\n${t('send_order_tip')}`);
      } else if (data) {
        setSpecialRequests(prev => [data as SpecialRequest, ...prev]);
        return true;
      }
    } catch (err: any) {
      console.error("Add special request exception:", err);
      alert(`${t('send_special_request_failed')}: ${err.message}`);
    }
    return false;
  };

  const updateSpecialRequest = async (id: string, updates: Partial<SpecialRequest>) => {
    const { error } = await withRetry(async () => await supabase.from('special_requests').update(updates).eq('id', id));
    if (error) {
      console.error("Error updating special request:", error);
      const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
      alert(`${t('update_request_failed')}: ${msg}\n\n${t('update_request_tip')}`);
    } else {
      setSpecialRequests(prev => prev.map(r => r.id === id ? { ...r, ...updates } : r));
    }
  };

  const updateSettings = async (newSettings: SellerSettings) => {
    const { error } = await withRetry(async () => await supabase.from('settings').upsert([{ id: 1, ...newSettings }]));
    if (error) {
      console.error("Error updating settings:", error);
      const msg = error.message === 'Failed to fetch' ? t('network_error') : error.message;
      alert(`${t('save_settings_failed')}: ${msg}\n\n${t('save_settings_tip')}`);
    } else {
      setSettings(newSettings);
      alert(t('settings_saved_success'));
    }
  };

  if (loading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white">
        <div className="w-12 h-12 border-4 border-black border-t-transparent rounded-full animate-spin"></div>
        <p className="mt-6 font-black text-black">{t('updating_data')}</p>
      </div>
    );
  }

  if (error) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-white p-6 text-center">
        <div className="text-6xl mb-6">📡</div>
        <h2 className="text-2xl font-black text-black mb-4">{t('connection_error')}</h2>
        <p className="text-red-600 font-bold mb-8 max-w-md">{error}</p>
        <button 
          onClick={() => { fetchInitialData(); }}
          className="px-8 py-4 bg-black text-white rounded-2xl font-black shadow-xl hover:scale-105 transition active:scale-95"
        >
          {t('retry')} 🔄
        </button>
      </div>
    );
  }

  return (
    <Router>
      <Layout>
        <Routes>
          <Route 
            path="/" 
            element={
              <BuyerGallery 
                products={products} 
                sellerSettings={settings} 
                onPurchase={addOrder}
                onSpecialRequest={addSpecialRequest}
              />
            } 
          />
          <Route 
            path="/login" 
            element={session ? <Navigate to="/seller" /> : <LoginPage />} 
          />
          <Route 
            path="/seller/*" 
            element={
              session ? (
                <SellerDashboard 
                  products={products} 
                  orders={orders} 
                  specialRequests={specialRequests}
                  settings={settings}
                  session={session}
                  onAddProduct={addProduct}
                  onDeleteProduct={deleteProduct}
                  onUpdateOrder={updateOrderStatus}
                  onUpdateSettings={updateSettings}
                  onUpdateSpecialRequest={updateSpecialRequest}
                  onRefresh={() => fetchInitialData(true)}
                />
              ) : (
                <Navigate to="/login" />
              )
            } 
          />
        </Routes>
      </Layout>
    </Router>
  );
};

export default App;
